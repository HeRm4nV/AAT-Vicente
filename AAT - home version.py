        ...
        for event in pygame.event.get():
            if event.type == KEYUP and event.key == K_ESCAPE:
                ...
                if actual_phase == 1:
                    ...
                    show_image(image_list[count], (base_size, base_size))
                    # sleepy_trigger(240 + (2 if condition == "sham" else 3) + (4 if image_list[count].split('\\')[2] == "N" else 0), lpt_address, trigger_latency) # Exposure image trigger second
                    ...
                    if count >= len(image_list):
                        ...
                        break
                # elif actual_phase == 6:
                    # slide(select_slide('exposure_' + condition),
                    #      False, K_SPACE, 2000)
                #    pygame.time.set_timer(phase_change, 2000, loops=1)
                #    actual_phase = 7
                elif actual_phase == 6:
                    screen.fill(background)
                    pygame.display.flip()
...
def create_image_list(condition, geometry):
    ...
    # , binge_images_list_first, control_images_list_first
