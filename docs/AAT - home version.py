...
def select_slide(slide_name, between_type = None, AAT_variables = {"block_number": 0, "geometry": "circle", "stick": "right", "practice": True}):
    ...
    basic_slides = {
        ...
        'Instructions_AAT': [
            ...
            " ",
            u"La tarea consiste en que cuando un ítem se presente con un " + ("CIRCULO" if AAT_variables["geometry"] == "circle" else "CUADRADO") + " debes ACERCAR",
            u"la imagen hacia ti, y cuando sea un " + ("CUADRADO" if AAT_variables["geometry"] == "circle" else "CIRCULO") + " debes ALEJAR la imagen de ti.",
            " ",
...
def wait_answer(key1, key2, limit_time = 0):
    ...
    #send_trigger(initial_trigger + 1, lpt_address, trigger_latency) # Inicio bloque (término en 1)
    tw = pygame.time.get_ticks()
    ...
    while switch:
        for event in pygame.event.get():
            ...
            if event.type == KEYUP:
                if event.key == key1:
                    #send_trigger(initial_trigger + 3, lpt_address, trigger_latency) # Barra espaciadora (término en 3)
                    answer_time = (pygame.time.get_ticks() - tw)
                if event.key == key2:
                    #send_trigger(initial_trigger + 3, lpt_address, trigger_latency) # Barra espaciadora (término en 3)
                    answer_time = (pygame.time.get_ticks() - tw)
            if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
    ...
    #send_trigger(initial_trigger + 2, lpt_address, trigger_latency) # Fin bloque (término en 2)
    return answers, block_type
...
def wait_control_answer(image, joystick, axis_number):
    ...
    while not done:
        for event in pygame.event.get():
            ...
            #    if ((pygame.time.get_ticks() - tw) > 2000):
            #        done=True
            #        break
    ...
    if (zoom_activate):
        if zoom_mode == "out":
            ...
            zoom(image = image, factor = 1.06, time = 0.6, iteration_speed = 10)
...
def show_images(image_list, condition):
    ...
    #image_list = attractive_images_list + neutral_images_list
        ...
        for event in pygame.event.get():
            if event.type == KEYUP and event.key == K_ESCAPE:
                ...
                if actual_phase == 1:
                    ...
                    pygame.time.set_timer(phase_change, 3500)
                    actual_phase = 2
                elif actual_phase == 2:
                    screen.fill(background)
                    pygame.display.update(fixbox)
                    ...
                    pygame.display.update(fixbox)
                    pygame.display.flip()
                    pygame.time.set_timer(phase_change, randint(1000,1300))
                    actual_phase = 5
                elif actual_phase == 5:
                    screen.fill(background)
                    pygame.display.flip()
                    ...
                    if (count % 20 != 0):
                        ...
                        pygame.time.set_timer(phase_change, 500)
                        actual_phase = 6
                    #pygame.time.set_timer(image_change, 600, True)
            ...
            '''
                ...
                if image and not message:
                    show_image(image_list[count], (base_size, base_size))
                    image = False
                    pygame.time.set_timer(image_change, 500)
                    #pygame.time.set_timer(image_change, 500, True)
                elif not image and not message:
                    screen.fill(background)
                    pygame.display.flip()
                    count += 1
                    if (count % 20 != 0):
                        pygame.time.set_timer(image_change, 600)
                        image = True
                    else:
                        pygame.time.set_timer(image_change, 300)
                        message = True
                    #pygame.time.set_timer(image_change, 600, True)
                    if count >= len(image_list):
                        pygame.time.wait(600)
                    ...
                    slide(select_slide('exposure_' + ("control" if condition == "C1" else "mindful")), False , K_SPACE, 2000)
                    pygame.time.set_timer(image_change, 2000)
                    image = True
                    message = False
            '''
    pygame.event.clear()                    # CLEAR EVENTS
...
def show_image_list(joystick, axis_number, image_list):
...
def main():
    ...
    #wait_answer(key1, key2, limit_time = 1000000):

    #send_trigger(start_trigger, lpt_address, trigger_latency)  # start EEG recording

    #play_video("VIDEO MINDFUL")
    #wait(K_SPACE, 0)

    #play_video("VIDEO CONTROL")
    #wait(K_SPACE, 0)
    ...
    #show_image_list(joystick, axis_number, image_list[0])
    ...
    show_image_list(joystick, axis_number, image_list[1])
    ...
    show_image_list(joystick, axis_number, image_list[2])
    ...
    show_image_list(joystick, axis_number, image_list[3])
    ...
    # Bloque 1
    slide(slides["intructions_" + design_type] , False , K_RETURN)
    answers, block_type = wait_answer(K_SPACE, times = 10, limit_time = 1200000, activate_sound = (design_type == "ext"), block_number = 1)
    answers_final_array.append(answers)
    ...
    if dfile != None:
        for count, line in enumerate(answers_final_array):
            print(block_types[count])
            for element in line:
                if isinstance(element, list):
                    dfile.write("%s,%s,%s,%s\n" % (subj_name, block_types[count], element[0], element[1]))
                else:
                    dfile.write("%s,%s,%s\n" % (subj_name, block_types[count], element))
